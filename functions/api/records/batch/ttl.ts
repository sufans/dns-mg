// POST /api/records/batch/ttl - Batch update record TTL
...
export const onRequestPost: PagesFunction = withCors(
  requireAuth(async (context: AuthenticatedEventContext) => {
    ...
    const parsed = BatchTtlSchema.safeParse(body);
    if (!parsed.success) {
      const firstError = parsed.error.issues[0];
      return createResponse(null, 400, firstError?.message ?? '输入验证失败');
    }

    const { accountId, domainId, recordIds, ttl } = parsed.data;
    ...
    try {
      ...
      await logOperation(context.env, {
        action: 'batch_ttl',
        targetType: 'record',
        detail: { accountId, domainId, recordIds, ttl },
        ipAddress: ip,
        userAgent: userAgent ?? undefined,
        status: 'success',
      });

      return createResponse({ recordIds, ttl }, 200, '批量修改TTL成功');
    } catch (error) {
      const message = error instanceof Error ? error.message : '批量修改TTL失败';
